"""
Extract total DOS from non-spin-polarized DOSCAR
"""
...
# Get total DOS at each energy
        DOS = float(all_PDOS[1])
        total_DOS[E_index] += DOS

    return np.asarray((energies,total_DOS))

dos_array = get_ion_PDOS(doscar,NEDOS,energies,E_f)

def plot_total_DOS(data_array,fermi_energy,E_lower,E_upper):
    # Set range of energy axis
    # Set energy data as a variable for neatness
    E = data_array[0]
    # Plot the DOS
    plt.plot(data_array[1],E)

    
    plt.ylim(E_lower,E_upper)
    xmin,xmax = plt.xlim(0,10)
...
plot_total_DOS(dos_array,E_f,-5,5)
